...
class L293_base
	{
		public:
			...
			uint16_t 	getRawPWMDC() 	const ;					///< Returns the last set speed value without applying the offset
			uint16_t 	getPWMDC()		const ;					///< Return the effective last set speed value

			virtual void forward( uint16_t PWMDC = 0 ) = 0 ;
			virtual void back( uint16_t PWMDC = 0 ) = 0 ;
		...
		protected:
			...
			uint16_t RawPWMDC;	///< The duty-cycle of the PWM signal applied to the enablePin
